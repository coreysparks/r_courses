...
library(glmnet)
library(MASS)
fm<-glm(modcontra~region+factor(age)+livchildren+wantmore+rural+factor(educ), data = dtrain, family=binomial) 
summary(fm)
```
...
```{r}
y<-dtrain$modcontra
x<-model.matrix(~factor(region)+factor(age)+livchildren+rural+wantmore+factor(educ), data=dtrain)
x<-(x)[,-1]
...
b1<-bestglm(yx,IC = "AIC", family=binomial, method = "exhaustive")
b1$BestModels
summary(b1$BestModel)
```
...
### Logistic ridge regression
```{r}
x.test <- model.matrix(~factor(region)+factor(age)+livchildren+rural+wantmore+factor(educ), data=dtest)
x.test<-(x.test)[,-1]
