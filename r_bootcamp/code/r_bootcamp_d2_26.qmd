...
The `select()` verb allows you to keep only certain columns in a dataframe:
...
The `arrange()` verb allows you to sort based on one or more columns:
...
Since we've already summarized the data into one row per continent, `geom_col()` is the more idiomatic `ggplot2` choice here - it's equivalent to `geom_bar(stat = "identity")`, but makes it clearer that the heights are already-computed values rather than counts `ggplot2` needs to tally itself.
...
prb %>%
  ...
  tally()%>%
  ggplot()+
  geom_col(aes(y = n, x = continent, fill = continent))+
...
[Quarto](https://quarto.org/) is a relatively new publishing system built around the Pandoc engine. This allows you to create dynamic documents using R, Python or other languages, as well as reports, websites and books in various publishing formats (HTML, PDF, Word, etc) **This lesson is written in Quarto!**
...
Here is a short example of doing this, where I create 2 vectors, `x` and `y` and save them. I'm using `tempfile()` here to write these to a temporary location instead of your project folder, just for the purposes of this demonstration - in your own work you'll want to give these a real path and file name.
...
R uses packages to store functions that do different types of analysis, so we will need to install lots of different packages to do different things. There are over 20,000 different packages currently for R. These are hosted on one of a number of *repositories*, such as the Comprehensive R Archive Network, or CRAN, which is the official repository for R packages. Other locations where authors store packages include [R-Forge](https://r-forge.r-project.org/) and [BioconductoR](https://www.bioconductor.org/). Many authors host packages in [Github](https://github.com) repositories, especially for development purposes.

Packages are often organized into *Task Views*, which CRAN uses to organize packages into thematic areas. You can find a list of these Task Views [here](https://cran.r-project.org/web/views/). There is not a task view for Demography, but there are ones for [Econometrics](https://cran.r-project.org/web/views/Econometrics.html) and [Spatial Data](https://cran.r-project.org/web/views/Spatial.html) to name a few. Task views allow users to download a lot of thematically linked packages in a single command, through the package `ctv`, or Cran Task Views. You can install this package by typing:
...
Over these two sessions, we've gone from installing R and Rstudio to reading in data, summarizing it, visualizing it with `ggplot2`, and reshaping and summarizing it with `dplyr`. You've also seen the different file types you'll encounter in R, and where to look for packages beyond the ones we've covered here.

This is far from everything R can do, but it should be enough to get you comfortable using R for your coursework. As you keep using R, you'll run into questions that these two sessions didn't cover - when that happens, the R help system (`?function_name`), the [RStudio Community forum](https://community.rstudio.com/), and Stack Overflow's `[r]` tag are all good places to look for answers, and you're always welcome to reach out to me directly.

All materials from these sessions are available on [Github](https://github.com/coreysparks/r_courses/tree/master/r_bootcamp).
