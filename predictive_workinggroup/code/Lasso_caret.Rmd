...
cbind(coef(fm), rmod3$beta, rmod1$beta)
```
...
yt<-dtest$modcontra
xt<-model.matrix(~factor(region)+factor(age)+livchildren+rural+wantmore+factor(educ)+partnered+work+factor(wealth), data=dtest)
xt<-(xt)[,-1]
yt<-as.factor(ifelse(yt==1,"mod", "notmod"))


predp1<-predict(fit_logistic, newdata=x)
#predc1<- as.factor(ifelse(predp1 > mean(dtrain$modcontra, na.rm=T), "mod", "notmod"))
confusionMatrix(data = predp1,y, positive = "mod" )


predrp1<-predict(fit_logistic, newdata=xt)
confusionMatrix(data = predrp1,yt, positive = "mod" )
```
