...
rp1<-caret::train(modcontra~., 
           ...
           metric="ROC",
           method ="glm",
          tuneLength=10, #try 20 random values of the tuning parameters
           trControl=fitctrl)
...
rp3<-caret::train(modcontra~., 
           data=dtrain,
           metric="ROC",
           method ="knn",tuneGrid=expand.grid(k = c(30, 35, 40, 45, 50, 55, 60)),
          #tuneLength=10, #try 20 random values of the tuning parameters
           trControl=fitctrl)
...
rp2<-caret::train(modcontra~., 
           ...
           metric="ROC",
           method ="knn",
          tuneGrid=expand.grid(k = c(30, 35, 40, 45, 50, 55, 60)), #try 20 random values of the tuning parameters
           trControl=fitctrl)
...
,
           #family=binomial,
           t#uneGrid=c(k=3),trControl=fitctrl)
           #,
          #tuneLength=10, #try 20 random values of the tuning parameters
           #trControl=fitctrl)
...
predrp1<-predict(rp2, newdata=dtrain, type = "prob")
roc1<-roc(dtrain$modcontra, predrp1$Modcontra)
plot(roc1)
...
mycp<-rp2$pred$k==rp2$bestTune$k
selectedIndices <- rp2$pred$k==mycp
# Plot:
plot.roc(rp2$pred$obs[selectedIndices], rp2$pred$Modcontra[selectedIndices], grid=T)
