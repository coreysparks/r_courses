...
km<-kmeans(x = prbtrain[, c(-1:-2)],
           center = 3,
           nstart = 20)
km

library(ClusterR)
km2<-KMeans_rcpp(data=prbtrain[, c(-1:-2)], cluster=4, num_init = 10)
...
kmeans_fancy <- kmeans(scale(prbtrain[, c(-1:-2)]), 5, nstart = 100)

fviz_cluster(kmeans_fancy,
             data = scale(prbtrain[, c(-1:-2)]),
             geom = c("point"),
             ellipse.type = "convex")
```
...
```{r}
library(cluster)
gap_stat <- clusGap(scale(prbtrain[, c(-1:-2)]),
                    FUN = kmeans,
                    nstart = 30,
                    K.max = 10, B = 100)
...
```{r}
fviz_nbclust(scale(prbtrain[, c(-1:-2)]), kmeans,
...
prbtest$cluster<-as.factor(predict_KMeans(data=prbtest[, c(-1:-2)],
