...
If you're coming from SPSS and you've been using the button clicking method, be prepared for a steep learning curve. If you've been writing syntax in SPSS, you're at least used to having to code. There's a good book for SAS and SPSS users by Bob Meunchen and Univ. of Tennessee [here](https://www.amazon.com/SAS-SPSS-Users-Statistics-Computing/dp/1461406846), which may be of some help.

###R and Rstudio

The Rgui is the base version of R, but is not very good to program in. Rstudio is much better, as it gives you a true integrated development environement (IDE), where you can write code in on window, see results in others, see locations of files, see objects you've created
...
5) A good source for all things programming is the statistics branch of [Stack Exchange](https://stats.stackexchange.com), which has lots of contributed questions and answers, although many answers are either very snarky or wrong, so *caveat emptor*.
...
Whoops! What happened? This means that R can't calculate the mean becuase there's a missing value, which we saw before. We can tell R to automatically remove missing values by:
...
I can read it from my github repository directly by using the `load()` function combined with the `url()` function, to tell R that the file is on the web. If the file were, say, in my documents folder, I could likwise load the data from disk.
...
The `dplyr` library is a portion of a suite of libraries known as the [*tidyverse*](https://www.tidyverse.org), which are oriented towards reproducible, intellible coding for data science. There are too many things within the tidyverse to cover them all here, but I will introduce you to two aspects: 
...
In generaly, `ggplot()` has a few core statements.
...
There is als0 a nice geometry called `freqpoly` that will draw polygons instead of bars for a histogram. I will use this to produce histograms for each continent.
